...
GitHub clone/download만으로 전역 명령이 등록되지는 않는다. `scripts/install.zsh`를 한 번 실행하면 package directory를 만들고, `__HOME__/.local/bin`에 얇은 shim을 생성한다. 이후 어느 디렉토리에서나 `claude-litellm`, `codex-litellm` 등을 호출할 수 있다. shim은 native `claude`, `codex`, `opencode`를 대체하지 않는다. 설치기는 checkout 밖의 package 안에도 `scripts/uninstall.zsh`를 복사하므로, repo checkout을 지운 뒤에도 `ai-litellm uninstall` 또는 `__FABRIC_HOME__/scripts/uninstall.zsh`로 package/shim을 제거할 수 있다.
...
OpenCode도 native 실행과 별도로 `opencode-litellm` managed profile을 둔다. OpenCode는 generated-config adapter로 실행한다.
...
`plan`/`execute`의 route는 사람용 `reasons`/`risks`/`rejected` 문자열과 함께 agent용 `selectionReasons`/`riskDiagnostics`/`rejections` diagnostic 객체를 제공한다. diagnostic 객체는 `{code,message,details?}` 형식이며, orchestration code는 문자열 파싱 대신 `code`를 본다. `execute`의 실제 실행은 non-interactive one-shot prompt만 지원한다. Claude는 `-p` + session persistence off, Codex는 `exec --sandbox read-only`, OpenCode는 `run --format json`으로 낮춘다. 선택된 route가 billable이면 `--confirm-billable` 없이는 실행하지 않고 `errorCode: "billing.confirmation_required"`와 JSON error를 반환한다. 이 계층은 supervisor가 아니라 router/launcher다. 장기 세션, 터미널 소유권, harness별 격리와 provider secret 주입은 계속 기존 wrapper가 담당한다.
...
| harness metadata/compaction | Codex catalog, Claude auto compact, OpenCode `limit` | 실제 provider enforcement인지, 표시/compaction/accounting인지 구분한다. |
...
- GLM-5.2 output cap처럼 provider public metadata가 없는 값은 `owned-policy`로 표시한다.

`ai-litellm context observations [filter]`는 repo-managed seed와 local state cache를 합쳐 보여준다. seed에는 현재 두 하한 증거가 들어 있다. Claude `opus`→DeepSeek는 `>=211580` input token 통과를 기록하고, GLM은 과거 `>=204800` boundary 관측을 보존한다. enforcement는 provider-authoritative configured cap을 우선하므로 GLM은 여전히 `202752`를 보수적으로 쓴다. 새 관측은 비싼 probe runner를 자동 실행하지 않고 다음처럼 기록한다.
...
- **Harness intent**: Claude Code, Codex, OpenCode가 선택 모델과 effort/thinking 개념을 어떻게 해석하고 전달하는지. 예: Claude tier(`opus`/`sonnet`/`haiku`) + `--effort`, Codex `model_reasoning_effort`, OpenCode `run --variant`.
...
opencode-litellm Qwen3.5-9B-MLX-8bit-omlx run --agent plan --format json "Reply with exactly LOCAL_OK"
```
...
이 방식은 secret을 `__FABRIC_HOME__/state/ai-litellm/env`에 `0600`으로 저장한다. package 삭제 시 이 private env file도 함께 제거된다. `scripts/install.zsh`는 `LITELLM_MASTER_KEY`가 환경변수/env-file/Keychain에 없으면 자동으로 local master key를 생성해 같은 env file에 저장한다.
